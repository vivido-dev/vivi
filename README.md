...
- A local Vivi may fall back to CPAL when presenter audio is unsupported. Remote and vvmux sessions disable local fallback by default. `VIVID_AUDIO_FALLBACK=allow`
  explicitly selects local-device fallback; `deny` disables it even in a local session.
...
`libavutil`, and `libswresample`, plus Clang/libclang for bindings generated from the selected
FFmpeg headers. Runtime library majors must match those headers.
...
```bash
sudo apt install clang libclang-dev pkg-config libavformat-dev libavcodec-dev libavutil-dev libswresample-dev libasound2-dev
...
Current Vivid 1.5 vvmux is supported, including a remote vvmux session entered through vvssh.
...
Traces contain bounded metadata about Vivid 1.5 control and media activity, with payload bytes
and capability material excluded. Dry-run and trace modes do not read a root secret or open an
audio device.
